...
# grab one grain
def make_one_grain(
    grain_length, audio_data, sample_rate, write_out=False, output_file_name=None
):
    filelength = len(audio_data)
    starting_sample = (
        np.random.randint(0, filelength - grain_length)
        if filelength > grain_length
        else 0
    )
...
def make_one_repitched_grain(
    ...
    repitched_grain_np = librosa.effects.pitch_shift(
        grain_np, sr=sample_rate, n_steps=steps_to_shift_pitch
    )
...
def repeat_grain(grain, num_repeats, sr, write_out=False, output_file_name=None):
    repeated_grains = []
    ...
    if write_out:
        audio_output_path = os.path.join("generated", "grains", output_file_name)
        sf.write(audio_output_path, repeated_grains, sr, subtype="PCM_24")
        print("Saved repeated grains to /grains!")
    return repeated_grains, sr
