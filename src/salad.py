...
def noise_salad(audio_dir, length=10, max_length=1, write_out=False, output_file_name=None):
    ...
    while len(salad_left) < length * initial_sample_rate:
        ...
        minimum_grain_length = sr * 0.1 if len(audio_data) > sr * 0.1 else len(audio_data) * 0.5

        grain_length = np.random.randint(minimum_grain_length, sr * 1.5 * max_length) if len(audio_data) > sr * 1.5 * max_length else np.random.randint(minimum_grain_length, len(audio_data))
...
def accordion_salad(audio_dir, length=10, max_length=1, write_out=False, output_file_name=None):
    num_audio_files, buffer_objs = load_audio_dir(audio_dir)
    initial_sample_rate = buffer_objs[0]['sample_rate']
    ...
    while len(salad_left) < length * initial_sample_rate:
        ...
        grain1 = make_one_repitched_grain(grain_length, audio_data, sr)
        grain2 = make_one_repitched_grain(grain_length, audio_data, sr)
        salad_left.extend(grain1)
        salad_right.extend(grain2)
